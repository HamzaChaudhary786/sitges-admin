...
export const getCustomersAction = (): AppThunk<Promise<string>> => async (dispatch) => {
	try {
		...
		if (error && (error as AxiosError).response) {
			return (error.response.data.error as string) || 'Error Getting  Customer';
		}
...
export const getEnterpriseCustomersAction =
	(eId: string): AppThunk<Promise<string>> =>
		async (dispatch) => {
			try {
				const res = await axios.get<{ data: Customers[] }>(`${END_POINT}/admin/enterprise-users/E${eId}`, {
					headers: {
						'access-token': localStorage.getItem('jwt_access_token')
					}
				});
				...
				if (error && (error as AxiosError).response) {
					return (error.response.data.error as string) || 'Error Getting Enterprise Customer';
				}

				return 'Error Getting Enterprise Customer';
			}
		};
	...
	(body: Partial<Customers>): AppThunk<Promise<string>> =>
		async () => {
			try {
				...
				const { referralCode: code, enterpriseId: epId, ...visitInfo2 } = visitInfo;

				if (!livesInSitges) {
					const visitInformation = await axios.post(`${END_POINT}/user/visit`, visitInfo2, {
						headers: {
							'access-token': accessToken
						}
					});
